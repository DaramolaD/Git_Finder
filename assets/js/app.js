/**
 * @license MIT
 * @author Daniel.D <dandaramola.d@gmail.com>
 * @copyright dandaramola.d 2023
 */
...
const /** {NobeList} */ $tabBtns = document.querySelectorAll("[data-tab-btn]");

const /** {NobeList} */ $tabPanels = document.querySelectorAll("[data-tab-panel]");
...
let /** {String} */ apiUrl = "https://api.github.com/users/daramolad";
...
window.updateProfile = function (profileUrl) {
    ...
    fecthData(profileUrl, data => {
        ...
        $profileCard.innerHTML = `
            <figure class="${type === "User" ? "avatar-circle" : "avatar-rounded"} img-holder" style="--width: 280;
                --height: 280">
            ...
            <a href="${githubPage}" target="_blank" class="btn btn-secondary">
                <span class="material-symbols-rounded" aria-hidden>open_in_new</span>
            ...
            <ul class="profile-meta">
                ...
                ${location ? 
                    `<li class="meta-item">
                        <span class="material-symbols-rounded" aria-hidden>location_on</span>
                        <span class="meta-text">${location}</span>
                    </li>` : ""
                }
                ${company ? 
                    `<li class="meta-item">
                        <span class="material-symbols-rounded" aria-hidden>apartment</span>
                        <a href="#" target="_blank" class="meta-text">${company}</a>
                    </li>` : ""
                }
                ${website ? 
                    `<li class="meta-item">
                        <span class="material-symbols-rounded" aria-hidden>captive_portal</span>
            ...
            <ul class="profile-stats">
                ...
                <li class="stats-item">
                    <span class="body">${numberToKilo(followers)}</span>Fellowers
                </li>
                <li class="stats-item">
                    <span class="body">${numberToKilo(following)}</span>Fellowing
                </li>
            </ul>
            <div class="footer"><p class="copyright">&copy; 2023 dandaramola</p></div>`
        ;
...
const updateRepository = function () {
    ...
    fecthData(`${repoUrl}?sort=create&per_page=12`, function (data) {
            ...
            for (const repo of repositories) {
                ...
                $repoCard.innerHTML = `
                    ...
                    <div class="card-footer">
                        ${language ? 
                            `<div class="meta-item">
                                <span class="material-symbols-rounded" aria-hidden>code_blocks</span>
                                <span class="span">${language}</span>
                            </div>` : ""
                        }

                        <div class="meta-item">
                            <span class="material-symbols-rounded" aria-hidden>star_rate</span>
                            <span class="span">${numberToKilo(stars_count)}</span>
                        </div>
                        <div class="meta-item">
                            <span class="material-symbols-rounded" aria-hidden>family_history</span>
...
const updateFollower = function () {
    ...
    $followerPanel.innerHTML = `
        <div class="card fellower-skeleton">
            <div class="skeleton avatar-skeleton"></div>
            <div class="skeleton title-skeleton"></div>
        </div>
    `.repeat(12);

    fecthData(followerUrl, function(data) {

        $followerPanel.innerHTML = `<h2 class="sr-only">Fellowers</h2>`;
        ...
        if(data.length) {
            for (const item of data) {
                ...
                $followerCard.classList.add("card", "fellower-card");

                $followerCard.innerHTML = `
                    <figure class="avatar-circle img-holder">
                        <img src="${avatar_url}" width="56" height="56" loading="lazy" 
...
const updateFollowing = function () {
    ...
    fecthData(followingUrl, function (data) {

        $followingPanel.innerHTML = `<h2 class="sr-only">Fellowing/h2>`;
        ...
        if(data.length) {
            for (const item of data) {
                ...
                $followingCard.classList.add("card", "fellower-card");
